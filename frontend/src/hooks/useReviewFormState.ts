...
import type { ArtistWithStats, Genre, Issue, MachineType, Song } from "../types";

type ReviewSong = Pick<Song, "id" | "title" | "youtubeUrl" | "generatedYoutube">;
...
type ReviewFormAction =
    | { type: "SET_FIELD"; field: keyof ReviewFormState; value: ReviewFormState[keyof ReviewFormState] }
...
function reducer(state: ReviewFormState, action: ReviewFormAction): ReviewFormState {
    switch (action.type) {
        case "SET_FIELD":
...
export function useReviewFormState(defaultMachineType?: MachineType) {
    const [state, dispatch] = useReducer(reducer, defaultMachineType, (mt) => initialState(mt));
    ...
    const applyOcrData = useCallback(
        (data: Partial<ReviewFormState>) => dispatch({ type: "OCR_IMPORT", payload: data }),
        [],
    );
