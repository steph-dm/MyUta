...
const Dashboard = () => {
  ...
  const { loading: statsLoading, error: statsError, data: statsData } = useQuery(DASHBOARD_STATS, {
    skip: !currentUser?.id,
      ...
      <div className="space-y-6">
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-16 text-center">
            <MaterialIcon icon="mic" size={64} className="text-muted-foreground/30 mb-4" />
            <h2 className="text-xl font-semibold mb-2">{t("welcome", { name: currentUser.name ?? "" })}</h2>
  ...
  const maxIssueCount = stats.commonIssues.length > 0
    ? stats.commonIssues[0].count
    : 0;
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        <Card>
          <CardHeader className="pb-2">
            ...
            <CardTitle className="text-xl sm:text-2xl">{stats.totalReviews}</CardTitle>
          </CardHeader>
        </Card>

        {sections.map(({ type }) => {
          const avg = type === "DAM" ? stats.damAvgScore : stats.joysoundAvgScore;
            ...
            <Card key={type}>
              <CardHeader className="pb-2">
                <CardDescription className="flex items-center gap-1 flex-wrap">
                  ...
                  <Badge variant="secondary" className={`${getMachineBadgeColor(type)} text-xs px-2 py-0`}>
                    {type}
                  </Badge>
                </CardDescription>
                <CardTitle className="text-xl sm:text-2xl">{avg.toFixed(3)}</CardTitle>
              </CardHeader>
            </Card>
          );
        })}
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardDescription className="flex items-center gap-1">
              <MaterialIcon icon="calendar_month" size={14} className="inline-block" /> {t("sessions30Days")}
            </CardDescription>
            <CardTitle className="text-xl sm:text-2xl">{stats.sessionsThisMonth}</CardTitle>
          </CardHeader>
          <CardContent className="pt-0">
            {stats.sessionsPrevMonth > 0 ? (
              <p className={`text-xs ${sessionsDiff > 0 ? "text-green-600" : sessionsDiff < 0 ? "text-red-600" : "text-muted-foreground"}`}>
                {sessionsDiff > 0 ? "↑" : sessionsDiff < 0 ? "↓" : "→"} {Math.abs(sessionsDiff)} {t("vsPrevious30Days")}
              </p>
            ) : stats.sessionsThisMonth > 0 ? (
              <p className="text-xs text-muted-foreground">{t("noPriorData")}</p>
            ) : (
      ...
      {stats.mostPracticed && stats.mostPracticed.count > 1 && (
        <Card>
          <CardContent className="flex items-center gap-3 py-3">
            <MaterialIcon icon="mic" size={16} className="text-primary shrink-0" />
            <span className="text-sm text-muted-foreground">
              {t("mostPracticed")}: <Link to={`/songs/${stats.mostPracticed.songId}`} className="font-medium text-foreground hover:underline transition-colors">{stats.mostPracticed.title}</Link> {t("by")} <span className="font-medium text-foreground">{stats.mostPracticed.artistName}</span> ({stats.mostPracticed.count} {t("reviews")})
            </span>
          </CardContent>
        </Card>
      )}
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm text-muted-foreground">
                    {reviews.length === 0
                      ? t("noRecentReviews")
                      : t("reviewCount", { count: reviews.length, ns: "reviews" })}
                  </span>
                </div>
              </button>
              {!(collapsedSections[type] ?? reviews.length === 0) && (
          ...
          <CardContent className="space-y-2">
            {stats.commonIssues.map((is) => (
              <div key={is.issue} className="flex items-center gap-3">
                <span className={`text-xs font-medium px-2 py-0.5 rounded border ${getIssueColor(is.issue as Issue)} w-[110px] whitespace-nowrap text-center shrink-0`}>
                ...
                <span className="text-xs text-muted-foreground w-6 text-right">{is.count}</span>
              </div>
            ))}
          </CardContent>
        </Card>
