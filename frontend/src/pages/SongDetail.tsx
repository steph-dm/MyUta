...
import {
  getGenreColor,
  getMachineBadgeColor,
  getMachineButtonColor,
} from "../lib/utils";
...
const SongDetail = () => {
  ...
  const joysoundReviews = reviews.filter(
    ...
    <div className="space-y-6">
              ...
              <div className="flex items-start justify-between gap-2">
                ...
                <div className="flex items-center gap-1 shrink-0">
                  {song.youtubeUrl && (
                    <Button
                      variant={
                        activePlayerUrl === song.youtubeUrl
                          ? "default"
                          : "outline"
                      }
                      size="sm"
                  ...
                  >
                    <Heart
                      className={`h-4 w-4 ${song.isFavorite ? "fill-current" : ""}`}
                    />
                  </Button>
                </div>
              </div>
          ...
          {[
            {
              label: "DAM",
              reviews: damReviews,
              color: getMachineButtonColor("DAM"),
              iconColor: "text-blue-600 dark:text-blue-400",
            },
            {
              label: "JOYSOUND",
              reviews: joysoundReviews,
              color: getMachineButtonColor("JOYSOUND"),
              ...
              return (
                <div key={section.label} className="space-y-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-muted-foreground">
                      {tReviews("reviewCount", {
                        count: section.reviews.length,
                      })}
                    </span>
                  </div>
                    ...
                    <Card>
                      <CardContent className="p-3 sm:pt-6 sm:px-6">
                        <div className="flex flex-col items-center text-center sm:flex-row sm:text-left sm:items-center gap-1 sm:gap-2">
                          <BarChart3
                            className={`h-4 w-4 sm:h-5 sm:w-5 ${section.iconColor}`}
                          />
                          <div>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          {sections.map(
            ({ type, reviews: sectionReviews, color, borderColor }) => (
